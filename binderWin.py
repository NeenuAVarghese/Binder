...
# Referred from: http://stackoverflow.com/questions/36796744/convert-exe-to-hex-to-be-run-by-python
#http://stackoverflow.com/questions/22219670/create-text-file-of-hexadecimal-from-binary
##########################################################
def getHexDump(execPath):
    ...
    with open(execPath, 'rb') as f:
        retVal = '0x' + '0x'.join(pair[0]+pair[1] + ','
                                  for pair in pairwise(hexlify(f.read()).decode('utf-8')))
    retVal = retVal[:-1]
    if(retVal == None):
...
def generateHeaderFile(execList, fileName):
    ...
    headerFile = open(fileName, "w", encoding='utf8')
    ...
    for execPath in execList:
        hexValue = getHexDump(execPath)
        hexArray = hexValue.split(",")
        hexLength = len(hexArray)
        hexSize.append(hexLength)
        headerFile.write("\n new unsigned char["+ str(hexLength) + "]{"+hexValue+"}")
        i += 1
        if(i < progCount):
            headerFile.write(",")
    headerFile.write("};")
...
def compileFile(binderCppFileName, execName):
    ...
    compileOutput = Popen(["g++", "binderbackendWin.cpp", "-o", execName, "-std=gnu++11"])
